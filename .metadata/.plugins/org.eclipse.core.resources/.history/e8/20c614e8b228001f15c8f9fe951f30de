...
$("[role='comment_question_list'],[role='comment_answer_list']").off().on('click', function() {
	var _cnt = 0;
	var _area = "";
	var _tmpArr = this.dataset.uri.split("/");
	var _page = _tmpArr.pop();
	if (this.role == "comment_question_list") {
		var _wrtieArea = $("[class='comment_question_write']");
		if (_wrtieArea.css("display") == "none") {
			_wrtieArea.show();
		} else {
			_wrtieArea.hide();
		}
		_area = $("[class='comment_question_list']");
		_cnt = $("[role='question_commant_cnt']").text();
	} else {
